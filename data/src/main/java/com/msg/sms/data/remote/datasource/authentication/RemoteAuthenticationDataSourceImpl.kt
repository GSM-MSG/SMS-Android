...
class RemoteAuthenticationDataSourceImpl @Inject constructor(
    ...
    override suspend fun submitAuthenticationForm(formData: SubmitAuthenticationFormRequest): Flow<Unit> =
        flow {
    ...
    override suspend fun verifyAuthentication(): Flow<VerifyAuthenticationResponse> = flow {
        emit(SMSApiHandler<VerifyAuthenticationResponse>().httpRequest {
            authenticationApi.verifyAuthentication()
        }.sendRequest())
    }
}